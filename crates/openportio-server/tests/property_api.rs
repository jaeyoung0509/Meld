...
proptest! {
    ...
    fn internal_error_mapping_is_sanitized(message in "(?s).{1,64}") {
        ...
        let (status, Json(rest)) = map_domain_error_to_rest(OpenportioError::Internal(message.clone()));
        ...
        let grpc = map_domain_error_to_grpc(OpenportioError::Internal(message));
    ...
    fn validation_domain_error_mapping_preserves_bad_request(message in "(?s).{1,64}") {
        let (status, Json(rest)) = map_domain_error_to_rest(OpenportioError::Validation(message.clone()));
        prop_assert_eq!(status, StatusCode::BAD_REQUEST);
        prop_assert_eq!(rest.code, "validation_error");
        prop_assert!(!rest.message.is_empty());

        let grpc = map_domain_error_to_grpc(OpenportioError::Validation(message));
