...
pub fn map_domain_error_to_rest(err: MeldError) -> ApiError {
    match err {
        MeldError::Validation(message) => {
...
pub fn map_domain_error_to_grpc(err: MeldError) -> Status {
    match err {
...
mod tests {
    ...
    fn internal_domain_errors_are_sanitized_for_rest_clients() {
        let (status, Json(body)) =
            map_domain_error_to_rest(MeldError::Internal("db exploded".to_string()));
    ...
    fn internal_domain_errors_are_sanitized_for_grpc_clients() {
        let status = map_domain_error_to_grpc(MeldError::Internal("db exploded".to_string()));
