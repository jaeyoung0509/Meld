...
impl Default for Config {
    fn default() -> Self {
        Self {
            proto: PathBuf::from("crates/alloy-rpc/proto/service.proto"),
            includes: vec![PathBuf::from("crates/alloy-rpc/proto")],
...
fn message_schema(name: &str, message: &DescriptorProto, index: &DescriptorIndex) -> Value {
    ...
    if !oneof_map.is_empty() {
        schema["x-alloy-oneof"] = json!(oneof_map);
    }

    if is_map_entry(message) {
        schema["x-alloy-map-entry"] = json!(true);
    }

    schema["x-alloy-message"] = json!(name);
    schema
}
