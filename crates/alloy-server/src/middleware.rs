...
impl MiddlewareConfig {
    pub fn from_env() -> Self {
        Self {
            timeout_seconds: read_env("ALLOY_TIMEOUT_SECONDS").unwrap_or(DEFAULT_TIMEOUT_SECONDS),
            max_in_flight_requests: read_env("ALLOY_MAX_IN_FLIGHT_REQUESTS")
                .unwrap_or(DEFAULT_MAX_IN_FLIGHT_REQUESTS),
            max_request_body_bytes: read_env("ALLOY_REQUEST_BODY_LIMIT_BYTES")
                .unwrap_or(DEFAULT_REQUEST_BODY_LIMIT_BYTES),
            cors_allow_origins: parse_cors_allow_origins(env::var("ALLOY_CORS_ALLOW_ORIGINS").ok()),
        }
    }
}
