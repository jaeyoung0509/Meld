...
pub type AlloyError = MeldError;

pub trait GreetingEngine: Send + Sync {
    fn greet(&self, name: &str) -> MeldResult<String>;
}
...
impl GreetingEngine for StaticGreetingEngine {
    fn greet(&self, name: &str) -> MeldResult<String> {
        if name.trim().is_empty() {
            return Err(MeldError::Validation("name must not be empty".to_string()));
        }
...
impl AppState {
    ...
    pub fn greet(&self, name: &str) -> MeldResult<String> {
...
mod tests {
    ...
    fn greet_rejects_empty_name() {
        let state = AppState::local("meld-test");
        let result = state.greet("  ");
        assert!(matches!(result, Err(MeldError::Validation(_))));
    }
